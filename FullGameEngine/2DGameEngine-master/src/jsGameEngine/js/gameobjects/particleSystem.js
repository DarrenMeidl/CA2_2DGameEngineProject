...
class ParticleSystem extends GameObject {
  ...
  constructor(x, y, color, count, lifeDuration, emitDuration, size) {
    ...
    this.size = size; // Size of the particles.
  }
  ...
  emitParticles(deltaTime) {
    ...
    for (let i = 0; i < particlesToEmit && this.particlesEmitted < this.count; i++) {
      ...
      const particle = new Particle(this.x, this.y, Math.random() * this.size, Math.random() * this.size, this.color, lifeDuration);
